...
bool AudioEncoder::initEncoder(const FrameFormat format)
{
    ...
    _audioEnc->sample_fmt = getCodecPreferedSampleFmt(codec, AV_SAMPLE_FMT_S16);
...
bool AudioEncoder::addToFifo(const Frame &audioFrame)
{
    ...
    n = av_audio_fifo_write(_audioFifo, reinterpret_cast<void **>(&data), audioFrame.additionalInfo.audio.nbSamples);
...
void AudioEncoder::encode()
{
    ...
    while (av_audio_fifo_size(_audioFifo) >= _audioEnc->frame_size) {
        logger("");
        int ret;
        int32_t n;
        n = av_audio_fifo_read(_audioFifo, reinterpret_cast<void **>(_audioFrame->data), _audioEnc->frame_size);
...
void AudioEncoder::sendOut(AVPacket &pkt)
{
    ...
    frame.timeStamp = currentTime() * frame.additionalInfo.audio.sampleRate / 1000;
